...
async function getTransformers() {
    try {
        ...
        throw new Error('Transformers.js tidak tersedia. Jalankan "npm install" di folder swacana-cli.');
    }
}
// ─── Ollama Check ──────────────────────────────────────────────────────────
...
async function generateWithOllama(prompt, systemPrompt, config) {
    ...
    try {
        ...
        throw new Error(`Ollama gagal: ${err.message}. Coba install Ollama dari https://ollama.com`);
    }
}
// ─── Generate with Transformers.js ─────────────────────────────────────────
async function generateWithTransformers(prompt, systemPrompt, config, onProgress) {
...
export async function generate(prompt, systemPrompt, config, onProgress) {
    ...
    if (ollamaOk) {
        try {
            ...
            console.warn('[AI] Ollama gagal, fallback ke Transformers.js:', err.message);
...
export async function quickGenerate(prompt, systemPrompt) {
