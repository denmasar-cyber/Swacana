...
import { notes, browserData, getStats, getDb, closeDb } from './storage/db.js';
...
program
    ...
    if (options.export) {
        ...
        const exportDir = path.join(process.env.HOME || process.env.USERPROFILE || '~', '.swacana', 'notes');
...
program
    ...
    if (options.url) {
        ...
        if (page.content)
            try {
                summary = await quickGenerate(page.content.slice(0, 3000), BROWSER_SUMMARIZER_PROMPT);
            }
            catch { }
