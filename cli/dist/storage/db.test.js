...
describe('Storage Layer (sql.js)', () => {
    ...
    describe('Notes', () => {
        it('should create a note and return its id', () => {
            ...
            db.run('INSERT INTO notes (id, title, content, source_type, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)', [id, 'Test Note', 'Hello World', 'manual', now, now]);
        ...
        it('should get all notes ordered by updated_at DESC', () => {
            ...
            db.run('INSERT INTO notes (id, title, content, source_type, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)', [crypto.randomUUID(), 'Note 1', 'Content 1', 'manual', now, earlier]);
            db.run('INSERT INTO notes (id, title, content, source_type, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)', [crypto.randomUUID(), 'Note 2', 'Content 2', 'manual', now, now]);
        ...
        it('should search notes by title or content', () => {
            const now = new Date().toISOString();
            db.run('INSERT INTO notes (id, title, content, source_type, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)', [crypto.randomUUID(), 'AI Research', 'Machine learning basics', 'manual', now, now]);
            db.run('INSERT INTO notes (id, title, content, source_type, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)', [crypto.randomUUID(), 'Shopping List', 'Groceries', 'manual', now, now]);
        ...
        it('should update a note', () => {
            const id = crypto.randomUUID();
            const now = new Date().toISOString();
            db.run('INSERT INTO notes (id, title, content, source_type, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)', [id, 'Original', 'Original content', 'manual', now, now]);
        ...
        it('should delete a note', () => {
            const id = crypto.randomUUID();
            const now = new Date().toISOString();
            db.run('INSERT INTO notes (id, title, content, source_type, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)', [id, 'Delete me', 'Content', 'manual', now, now]);
    ...
    describe('Files', () => {
        it('should insert a file', () => {
            const id = crypto.randomUUID();
            const now = new Date().toISOString();
            db.run('INSERT INTO files (id, path, filename, extension, size_bytes, mtime, content_hash) VALUES (?, ?, ?, ?, ?, ?, ?)', [id, '/test/file.ts', 'file.ts', '.ts', 1024, now, 'abc123']);
        ...
        it('should count files', () => {
            const now = new Date().toISOString();
            db.run('INSERT INTO files (id, path, filename, extension, size_bytes, mtime, content_hash) VALUES (?, ?, ?, ?, ?, ?, ?)', [crypto.randomUUID(), '/a.ts', 'a.ts', '.ts', 100, now, 'a']);
            db.run('INSERT INTO files (id, path, filename, extension, size_bytes, mtime, content_hash) VALUES (?, ?, ?, ?, ?, ?, ?)', [crypto.randomUUID(), '/b.ts', 'b.ts', '.ts', 200, now, 'b']);
    ...
    describe('Chunks', () => {
        it('should create chunks with file reference', () => {
            ...
            db.run('INSERT INTO files (id, path, filename, extension, size_bytes, mtime, content_hash) VALUES (?, ?, ?, ?, ?, ?, ?)', [fileId, '/test.ts', 'test.ts', '.ts', 100, now, 'hash']);
            const chunkId = crypto.randomUUID();
            db.run('INSERT INTO chunks (id, file_id, note_id, text, token_count) VALUES (?, ?, ?, ?, ?)', [chunkId, fileId, null, 'chunk content', 25]);
            const row = queryById('chunks', chunkId);
            expect(row.text).toBe('chunk content');
            expect(row.token_count).toBe(25);
        });
        it('should count chunks', () => {
            db.run('INSERT INTO chunks (id, file_id, note_id, text, token_count) VALUES (?, ?, ?, ?, ?)', [crypto.randomUUID(), null, null, 'chunk 1', 10]);
            db.run('INSERT INTO chunks (id, file_id, note_id, text, token_count) VALUES (?, ?, ?, ?, ?)', [crypto.randomUUID(), null, null, 'chunk 2', 20]);
    ...
    describe('Embeddings', () => {
        it('should store and retrieve binary vector data', () => {
            ...
            db.run('INSERT INTO embeddings (id, chunk_id, note_id, vector, dim, model) VALUES (?, ?, ?, ?, ?, ?)', [id, chunkId, null, buffer, 5, 'test-model']);
            const row = queryById('embeddings', id);
            const storedBuffer = row.vector;
    ...
    describe('Browser Data', () => {
        it('should store browser captured data', () => {
            const id = crypto.randomUUID();
            const now = new Date().toISOString();
            db.run('INSERT INTO browser_data (id, title, url, content, summary, captured_at) VALUES (?, ?, ?, ?, ?, ?)', [id, 'Test Page', 'https://example.com', 'Page content', 'A summary', now]);
    ...
    describe('Agent Memory', () => {
        it('should create memory entries', () => {
            const id = crypto.randomUUID();
            const now = new Date().toISOString();
            db.run('INSERT INTO agent_memory (id, type, content, context, created_at) VALUES (?, ?, ?, ?, ?)', [id, 'insight', 'Important insight', '/some/context', now]);
        ...
        it('should get unactioned memories', () => {
            const now = new Date().toISOString();
            db.run('INSERT INTO agent_memory (id, type, content, context, created_at, is_actioned) VALUES (?, ?, ?, ?, ?, ?)', [crypto.randomUUID(), 'task', 'Task 1', null, now, 0]);
            db.run('INSERT INTO agent_memory (id, type, content, context, created_at, is_actioned) VALUES (?, ?, ?, ?, ?, ?)', [crypto.randomUUID(), 'task', 'Task 2', null, now, 1]);
            db.run('INSERT INTO agent_memory (id, type, content, context, created_at, is_actioned) VALUES (?, ?, ?, ?, ?, ?)', [crypto.randomUUID(), 'insight', 'Insight 1', null, now, 0]);
        ...
        it('should mark memory as actioned', () => {
            const id = crypto.randomUUID();
            const now = new Date().toISOString();
            db.run('INSERT INTO agent_memory (id, type, content, context, created_at, is_actioned) VALUES (?, ?, ?, ?, ?, ?)', [id, 'task', 'Action me', null, now, 0]);
    ...
    describe('Edge Cases', () => {
        it('should handle very large text in notes', () => {
            ...
            db.run('INSERT INTO notes (id, title, content, source_type, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)', [id, 'Large Note', largeText, 'manual', now, now]);
            const row = queryById('notes', id);
        ...
        it('should handle special characters in content', () => {
            ...
            db.run('INSERT INTO notes (id, title, content, source_type, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)', [id, 'Special', specialText, 'manual', now, now]);
            const row = queryById('notes', id);
        ...
        it('should handle concurrent inserts (50 rows)', () => {
            const now = new Date().toISOString();
            for (let i = 0; i < 50; i++) {
                db.run('INSERT INTO notes (id, title, content, source_type, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)', [crypto.randomUUID(), `Note ${i}`, 'Content', 'manual', now, now]);
            }
