...
import { FileWatcher, type FileChangeEvent } from '../fs/watcher.js';
...
function logWatch(msg: string) {
  console.log(`  [${timestamp()}] ${msg}`);
}

// ─── Handle File Change ────────────────────────────────────────────────────

async function handleChange(event: FileChangeEvent) {
  changeCount++;
  ...
  if (event.type !== 'unlink') {
    try {
      ...
      logWatch(`⚠️ Gagal proses: ${(err as Error).message}`);
    }
  }
}
