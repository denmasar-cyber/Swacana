 ...
 */
function findWebAppRoot(): string | null {
...
let collabProcess: ChildProcess | null = null;
...
function startCollabServer(port: number): void {
  if (!WEB_DIR) return;
  ...
  proc.stdout?.on('data', (d: Buffer) => {
    ...
    if (m) console.log(`   [collab] ${m}`);
  });

  proc.stderr?.on('data', (d: Buffer) => {
    const m = d.toString().trim();
    if (m && !m.includes('ExperimentalWarning')) console.log(`   [collab] ${m}`);
  });
...
function openBrowser(url: string): void {
  try {
    ...
    if (plat === 'win32') spawn('cmd', ['/c', 'start', url], { shell: true, detached: true });
    else if (plat === 'darwin') spawn('open', [url], { detached: true });
...
export function stopDashboard(): void {
  ...
  if (collabProcess) { try { collabProcess.kill('SIGTERM'); } catch {} collabProcess = null; }
}
