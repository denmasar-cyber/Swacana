...
import { scanFiles, type ScanResult } from '../fs/scanner.js';
import { initializeAgent } from '../agent/index.js';
import { embedText, splitIntoChunks, initEmbeddingEngine } from '../ai/embedding.js';
import { quickGenerate } from '../ai/engine.js';
import { SUMMARIZER_PROMPT } from '../ai/prompts.js';
import { files, chunks, notes, memories, getDb } from '../storage/db.js';
  ...
  for (let i = 0; i < filesToProcess.length; i++) {
    ...
    try {
      ...
      const fileId = files.upsert(
        ...
        file.extension,
        file.sizeBytes,
        (await fs.promises.stat(file.filePath)).mtime.toISOString(),
        file.contentHash || '',
      );
