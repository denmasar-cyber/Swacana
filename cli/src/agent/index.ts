...
import { notes, files, chunks, memories, getDb, type FileEntry } from '../storage/db.js';
import { storeEmbedding } from '../storage/vector.js';
import { FileChangeEvent, isTextFile, readTextFile } from '../fs/watcher.js';
...
export async function processFileChange(
  ...
  if (event.type === 'unlink') return insights;

  try {
    // Read file content
    const content = await readTextFile(event.filePath);
    if (!content || content.trim().length < 50) return insights;

    // Store file in database
    const stat = await fs.stat(event.filePath);
    const fileId = files.upsert(
      event.filePath,
      event.filename,
      event.extension,
      stat.size,
    ...
    const summary = await quickGenerate(
      `Ringkas file ini:\n${content.slice(0, 2000)}`,
      SUMMARIZER_PROMPT,
    );

    if (fileId) files.markIndexed(fileId, summary);

    // Split into chunks and embed
    const chunkResults = splitIntoChunks(content);
    const chunkIds = chunks.bulkCreate(
      chunkResults.map((c) => ({
        fileId,
        noteId: null,
        text: c.text,
    ...
    // Extract insight
    const insight = await quickGenerate(
    ...
    if (content.length > 500) {
      try {
        const noteContent = await quickGenerate(
          `Buat catatan dari file ini:\n${summary}\n\nSource: ${event.filePath}`,
          NOTE_CREATOR_PROMPT,
        );
        ...
        if (noteContent.trim()) {
          ...
          notes.create(
            `📄 ${title}`,
            `Auto-generated from: ${event.filePath}\n\n${noteContent}`,
            'file',
          );
        }
    ...
    console.warn(`[Agent] Gagal proses ${event.filePath}:`, (err as Error).message);
  }
...
export async function consolidateMemories(): Promise<string | null> {
...
export function createNoteFromInsight(
  ...
  try {
    ...
    const noteId = notes.create(
      title.slice(0, 80),
      `**Source:** ${insight.context || 'Auto-generated'}\n\n${insight.content}`,
      'auto',
    );
...
export async function runAgentOnce(
  ...
  if (allInsights.length > 0) {
    ...
    notes.create(
      `📝 Ringkasan AI - ${new Date().toLocaleDateString('id-ID')}`,
      combined,
      'auto',
    );
